...
| Client custom headers forwarded to backends | Low | `mcp-session-id` replaced with specific target mcp backend session; `mcp-init-host` and `router-key` are stripped by the gateway HTTPRoute before traffic reaches a backend | Review header forwarding policy if backends are untrusted |
